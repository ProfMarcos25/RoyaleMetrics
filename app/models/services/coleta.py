"""
ServiÃ§o de Coleta de Dados â€” Royle Metrics
ResponsÃ¡vel por buscar dados na API do Clash Royale e
...
from app.models.services.clash_client import get_client
...
def _normalizar_tag(tag: str) -> str:
    """
    Garante que a tag comeÃ§a com '#' e estÃ¡ em maiÃºsculas.
...
def _gerar_battle_id(player_tag: str, battle_time: str) -> str:
    """
    Gera um identificador Ãºnico para cada batalha combinando
    a tag do jogador com o horÃ¡rio da batalha via SHA-256.
    Evita inserÃ§Ãµes duplicadas mesmo em coletas repetidas.

    Args:
        player_tag: Tag do jogador (ex: '#ABC123').
        battle_time: HorÃ¡rio da batalha no formato da API.
...
def _url_icone_carta(nome: str) -> str:
    """
    Monta a URL do Ã­cone da carta usando o repositÃ³rio pÃºblico
    ...
    """
    # Converte espaÃ§os em hifens e coloca em minÃºsculas (padrÃ£o dos assets)
    nome_formatado = nome.lower().replace(" ", "-")
    return f"https://royaleapi.github.io/cr-api-assets/cards/{nome_formatado}.png"


# =============================================================
# FunÃ§Ãµes principais de sincronizaÃ§Ã£o
# =============================================================

def sincronizar_cartas(db: Session) -> int:
    """
    Busca todas as cartas do jogo via API e persiste no banco.
    Realiza upsert: atualiza se existir, insere se nÃ£o existir.
    Essa funÃ§Ã£o deve ser executada na primeira inicializaÃ§Ã£o do projeto.

    Args:
        db: SessÃ£o ativa do banco de dados SQLAlchemy.

    Retorna:
        int: NÃºmero de cartas sincronizadas.

    Erros tratados:
        - NotFoundError: endpoint invÃ¡lido (raro para /cards).
        - RatelimitError: muitas requisiÃ§Ãµes â€” aguarda prÃ³ximo ciclo.
    """
    client = get_client()
    logger.info("Iniciando sincronizaÃ§Ã£o de cartas...")

    try:
        cartas_api = client.get_cards()
    except clashroyale.errors.NotFoundError:
        logger.error("Endpoint de cartas nÃ£o encontrado na API.")
        return 0
    ...
    db.commit()
    logger.info(f"SincronizaÃ§Ã£o de cartas concluÃ­da: {contador} cartas processadas.")
    return contador
...
def sincronizar_clan(tag: str, db: Session) -> Optional[Clan]:
    ...
    - Dados gerais do clÃ£ (nome, descriÃ§Ã£o, trofÃ©us)
    - Lista de membros (jogadores)
    - Batalhas de cada membro

    Args:
        tag: Tag do clÃ£ (ex: '#ABC123').
        db: SessÃ£o ativa do banco de dados.

    Retorna:
        Clan: Objeto do clÃ£ sincronizado, ou None em caso de erro.

    Erros tratados:
        - NotFoundError: clÃ£ nÃ£o encontrado (tag invÃ¡lida).
        - RatelimitError: aguarda prÃ³ximo ciclo de coleta.
    """
    client = get_client()
    ...
    try:
        ...
        logger.error(f"ClÃ£ {tag} nÃ£o encontrado na API. Verifique a tag.")
        return None
    except clashroyale.errors.RatelimitError:
    ...
    db.flush()  # garante que clan.id esteja disponÃ­vel
...
def sincronizar_batalhas_jogador(tag: str, db: Session) -> int:
    ...
    Registra tambÃ©m as cartas usadas em cada batalha (deck).
    ...
    Retorna:
        int: NÃºmero de novas batalhas inseridas.

    Erros tratados:
        - NotFoundError: jogador nÃ£o encontrado.
        - RatelimitError: aguarda prÃ³ximo ciclo.
    """
    client = get_client()
    tag = _normalizar_tag(tag)
    ...
    if jogador is None:
        logger.warning(f"Jogador {tag} nÃ£o encontrado no banco. Pulando batalhas.")
        return 0

    try:
        batalhas_api = client.get_player_battles(tag)
    except clashroyale.errors.NotFoundError:
        logger.error(f"Jogador {tag} nÃ£o encontrado na API.")
        return 0
    except clashroyale.errors.RatelimitError:
    ...
    for battle_api in batalhas_api:
        ...
        try:
            ...
            for carta_api in cartas_deck:
                ...
                if carta is None:
                    # Insere carta desconhecida para nÃ£o perder o registro
                    carta = Carta(
            ...
            pass  # deck indisponÃ­vel para este tipo de batalha

        # Atualiza timestamp da Ãºltima batalha do jogador
        if data_batalha and (
...
def sincronizar_warlog(tag: str, db: Session) -> int:
    """
    Busca o histÃ³rico de guerras do clÃ£ e persiste no banco.
    Inclui desempenho individual dos membros (contribuiÃ§Ãµes).
    ...
    Retorna:
        int: NÃºmero de guerras processadas.

    Erros tratados:
        - NotFoundError: clÃ£ nÃ£o encontrado.
        - RatelimitError: aguarda prÃ³ximo ciclo.
    """
    client = get_client()
    tag = _normalizar_tag(tag)
    ...
    clan = db.query(Clan).filter(Clan.tag == tag).first()
    if clan is None:
        logger.warning(f"ClÃ£ {tag} nÃ£o encontrado no banco. Execute sincronizar_clan primeiro.")
        return 0

    try:
        warlog_api = client.get_clan_warlog(tag)
    except clashroyale.errors.NotFoundError:
        logger.error(f"ClÃ£ {tag} nÃ£o encontrado na API (warlog).")
        return 0
    except clashroyale.errors.RatelimitError:
    ...
    for war_api in warlog_api:
        ...
        # Verifica se essa guerra jÃ¡ foi registrada
        ...
        # Encontra a posiÃ§Ã£o do clÃ£ nos standings
        ...
        # Registra contribuiÃ§Ãµes individuais
...
def sincronizar_river_race(tag: str, db: Session) -> Optional[Guerra]:
    ...
    Diferente do warlog, esta funÃ§Ã£o captura dados da guerra ativa,
    que ainda nÃ£o foi concluÃ­da.
    ...
    clan = db.query(Clan).filter(Clan.tag == tag).first()
    if clan is None:
        logger.warning(f"ClÃ£ {tag} nÃ£o encontrado no banco.")
        return None

    try:
        race_api = client.get_clan_current_river_race(tag)
    except clashroyale.errors.NotFoundError:
        logger.warning(f"ClÃ£ {tag} nÃ£o estÃ¡ em uma guerra ativa no momento.")
        return None
    except clashroyale.errors.RatelimitError:
    ...
    # Processa participantes e suas contribuiÃ§Ãµes
    ...
    db.commit()
    logger.info(f"River Race do clÃ£ {tag} sincronizado. PontuaÃ§Ã£o atual: {guerra.pontuacao}.")
    return guerra
